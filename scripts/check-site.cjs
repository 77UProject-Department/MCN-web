...
for (const f of files.filter((f) => /\.(html|js|css)$/.test(f))) {
  ...
  for (const match of text.matchAll(
    /(?:src=["']|href=["']|url\(["']|["'])(\/(?:assets\/[^'"\s)]+|styles\.css|content\.js|app\.js))/g,
  )) {
    assert.ok(
      fs.existsSync(path.join(root, match[1])),
      `Missing asset ${match[1]} in ${f}`,
    );
    checked++;
  }
}
for (const b of S.businesses)
  assert.ok(
    fs.existsSync(path.join(root, b.id, "index.html")),
    `Missing ${b.id}`,
  );
for (const n of S.news)
  assert.ok(
    fs.existsSync(path.join(root, "news", n.id, "index.html")),
    `Missing news ${n.id}`,
  );
for (const src of [S.hero.image, ...S.businesses.map((b) => b.image)])
  assert.ok(fs.existsSync(path.join(root, src)), `Missing image ${src}`);
...
const renderPaths = [
  ...
  ...files
    .filter(
      (f) => f.endsWith("index.html") && f !== path.join(root, "index.html"),
    )
    .map(
      (f) =>
        "/" + path.relative(root, path.dirname(f)).split(path.sep).join("/"),
    ),
];
for (const route of renderPaths) {
  ...
  const sandbox = {
    window: { SITE: S, addEventListener() {}, scrollY: 0 },
    document,
  ...
  const html = ["#header", "#main", "#footer"]
    .map((id) => elements.get(id)?.innerHTML || "")
    .join("");
  ...
  for (const [, id] of html.matchAll(/href="#([^"]+)"/g))
    assert.ok(html.includes(`id="${id}"`), `Broken anchor #${id} on ${route}`);
}
console.log(
  `OK: ${renderPaths.length} route renderers execute; rendered links, anchors and image references resolve.`,
);
assert.ok(
  !/无忧传媒|wuyou\.com|浙ICP备2024076499|18500870034/.test(
    fs.readFileSync(path.join(root, "app.js"), "utf8") +
      fs.readFileSync(path.join(root, "content.js"), "utf8"),
  ),
  "Reference company data found in site content",
);
console.log(
